...
public class CommonStepsDef extends AbstractStepsDef{
    ...
    @And("^I change region of site to \"([^\"]*)\"$")
    public void iChangeRegionOfSiteTo(String region) {
        if (!homePage.getRegionName().equals(region)) {
            homePage.changeRegion(region);
            attachments.attachScreenShot("After region changes");
        }
    }
}
